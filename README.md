...
Compatibility currently permits only Foundry 14. We intentionally omit
`compatibility.verified` until we have tested it inside Foundry; a compatibility
warning is therefore expected. After a successful smoke test, set `verified`
to the exact Foundry build tested, for example `14.365` if that is your build.
...
- `download`: the ZIP for this specific module version. For `0.1.0`, this points
  to the `module.zip` asset attached to GitHub release tag `v0.1.0`.
...
2. For each subsequent release, increase `version` in `module.json` and update
   its `download` URL to the matching tag (for example `v0.1.1`). Keep the
   `manifest` URL stable.
...
4. On GitHub, create a release with tag `v0.1.0` (or your new version) targeting
