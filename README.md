...
The module version (`0.2.3`) is independent of the Foundry version (`14`).
...
- `download`: the ZIP for this specific module version. For `0.2.3`, this points
  to the `module.zip` asset attached to GitHub release tag `v0.2.3`.
...
2. For each subsequent release, increase `version` in `module.json` and update
   its `download` URL to the matching tag (for example `v0.2.3`). Keep the
   `manifest` URL stable.
...
4. On GitHub, create a release with tag `v0.2.3` (or your new version) targeting
